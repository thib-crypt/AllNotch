...
class DetachedEditorWindowController: NSObject, NSWindowDelegate {
    ...
    private func show(image: NSImage, tool: AnnotationTool?, color: NSColor?, strokeWidth: CGFloat?, annotations: [Annotation]) {
        ...
        win.title = "AllNotch Editor — \(dateFmt.string(from: Date()))"
    ...
    func windowWillClose(_ notification: Notification) {
        ...
        if Self.activeControllers.isEmpty {
            MacshotManager.shared.returnFocusIfNeeded()
        }
    }
    ...
    private func saveToHistory() {
        ...
        MacshotManager.shared.refreshThumbnail(for: entryID, image: finalImage)
    }
    ...
    private func commitToHistory() {
        ...
        if let image = finalImage, let entryID = historyEntryID {
            MacshotManager.shared.showFloatingThumbnail(image: image, historyEntryID: entryID)
        }
    }
...
extension DetachedEditorWindowController: OverlayViewDelegate {
    ...
    func overlayViewDidConfirm() {
        ...
        playCopySound()
        MacshotManager.shared.showFloatingThumbnail(image: image)
    ...
    func overlayViewDidRequestUpload() {
        ...
        let image = applyPostProcessing(raw)
        playCopySound()
        MacshotManager.shared.showUploadProgress(image: image)
        autoSaveToHistoryIfNeeded(compositedImage: image)
    }
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            do {
                ...
                DispatchQueue.main.async {
                    ...
                    self.playCopySound()
                    MacshotManager.shared.showFloatingThumbnail(image: finalImage)
                }
            } catch {}
        }
    }
