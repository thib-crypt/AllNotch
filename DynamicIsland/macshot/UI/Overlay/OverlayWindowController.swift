...
/// How a capture was finalised by the overlay, so the manager knows whether the
/// floating vignette should arm a *deferred* default action.
///
/// - `.copied` / `.saved`: the user took an explicit disposition (Copy or Save
///   button); the vignette is purely informational and arms nothing.
/// - `.deferred`: no explicit disposition. The vignette arms the user's
///   "Default Action" (`quickCaptureMode`), applied only if the vignette
///   auto-dismisses without interaction.
...
class OverlayWindowController {
    ...
    private func playCopySound() {
        ...
        MacshotManager.captureSound?.stop()
        MacshotManager.captureSound?.play()
    }
...
extension OverlayWindowController: OverlayViewDelegate {
    ...
    func overlayViewDidConfirm() {
        ...
        overlayDelegate?.overlayDidConfirm(self, capturedImage: finalImage, annotationData: annotationData, disposition: .copied)
    }
    ...
    func overlayViewDidRequestShare(anchorView: NSView?) {
        ...
        let delegate = SharePickerDelegate(
            onPick: { [weak self] in
                ...
                let img = image
                self.dismiss()
                self.overlayDelegate?.overlayDidConfirm(self, capturedImage: img, annotationData: nil, disposition: .copied)
            },
    ...
    func overlayViewDidRequestRemoveBackground() {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            do {
                ...
                DispatchQueue.main.async {
                    // Disposition is deferred — the floating vignette applies the
                    // user's Default Action (quickCaptureMode) if it auto-dismisses.
                    self.playCopySound()
                    self.dismiss()
                    self.overlayDelegate?.overlayDidConfirm(self, capturedImage: finalNSImage, annotationData: nil, disposition: .deferred)
                }
            } catch {
                #if DEBUG
    ...
    func overlayViewDidRequestQuickSave() {
        ...
        // Quick capture is a *deferred* finalisation: no copy/save happens now.
        // The floating vignette arms the user's Default Action (quickCaptureMode)
        // and applies it only if the vignette auto-dismisses without interaction.
        playCopySound()
        overlayDelegate?.overlayDidConfirm(self, capturedImage: image, annotationData: annotationData, disposition: .deferred)
    }
    ...
    func overlayViewDidRequestFileSave() {
        ...
        overlayDelegate?.overlayDidConfirm(self, capturedImage: image, annotationData: nil, disposition: .saved)
    ...
    func overlayViewDidRequestSave() {
        ...
        savePanel.begin { [weak self] response in
            ...
            if response == .OK, let url = savePanel.url {
                ...
                self.playCopySound()
                self.dismiss()
                self.overlayDelegate?.overlayDidConfirm(self, capturedImage: nil, annotationData: nil, disposition: .saved)
            } else {
