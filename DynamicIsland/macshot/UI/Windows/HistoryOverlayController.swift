...
final class HistoryOverlayController: NSObject, QLPreviewPanelDataSource, QLPreviewPanelDelegate {
    ...
    /// Keeps the controller alive while the overlay is on screen. The call site
    /// (`HistoryOverlayController().show()`) holds no reference, and neither the
    /// windows' `weak var controller` nor the classic NotificationCenter observer
    /// retain us — without this the controller deallocates the moment `show()`
    /// returns, leaving the backdrop on screen with every action wired to a nil
    /// controller (clicks/ESC no-op, input trapped). Cleared in `dismiss()`.
    private var selfRetain: HistoryOverlayController?
    /// Guards against re-entrant/double dismissal (e.g. a click that triggers
    /// `dismiss()` followed by `appDidResignActive`).
    ...
    func show() {
        ...
        // Own ourselves for the lifetime of the overlay (see `selfRetain`).
        selfRetain = self
    ...
    func confirmClearHistory() {
        ...
        MainActor.assumeIsolated {
            MacshotManager.shared.confirmClearHistory()
        }
    }
    ...
    /// Drop the self-reference on the next runloop turn so `self` is never
    /// deallocated mid-method (which would dangle `self` for any code after the
    /// nil-out). Once released, the controller deallocates as expected.
    private func releaseSelf() {
        DispatchQueue.main.async { [weak self] in self?.selfRetain = nil }
    }
    ...
    func pinToScreen(index: Int) {
        ...
        dismiss()
        DispatchQueue.main.async {
            MacshotManager.shared.showPin(image: image)
        }
    }
...
private final class BackdropView: NSView {
    ...
    // Deliver the click even when AllNotch (an accessory app) isn't frontmost,
    // so the first click on the backdrop dismisses instead of being swallowed
    // to activate the app. Matches the capture overlay / toolbar views.
...
private final class HistoryPanelView: NSView, NSDraggingSource {
    ...
    // See BackdropView: deliver clicks even when the accessory app isn't
    // frontmost so a card click registers on the first press.
    override func acceptsFirstMouse(for event: NSEvent?) -> Bool { true }
