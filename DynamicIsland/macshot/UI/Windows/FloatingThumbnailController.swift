...
class FloatingThumbnailController: NSObject, NSDraggingSource {
    ...
    /// Fired ONLY when the vignette auto-dismisses on its timer without any user
    /// interaction. Used to apply the deferred Default Action (save/copy). Any
    /// explicit action or manual close cancels the timer, so this never fires then.
    var onTimeout:    (() -> Void)?
    ...
    func show(at origin: NSPoint, corner: FloatingThumbnailCorner) {
        ...
        view.onAddToShelf = { [weak self] in self?.onAddToShelf?(); self?.dismiss() }
    ...
    private func scheduleAutoDismiss() {
        ...
        let task = DispatchWorkItem { [weak self] in
            // Timer expired with no interaction → apply the deferred Default Action.
            self?.onTimeout?()
            self?.onTimeout = nil
            self?.animateOut()
        }
...
private class ThumbnailView: NSView {
    ...
    private var shelfBtnRect:  NSRect = .zero
    ...
    override func mouseMoved(with event: NSEvent) {
        ...
        let rects = [closeBtnRect, pinBtnRect, editBtnRect, uploadBtnRect, copyBtnRect, saveBtnRect, shelfBtnRect]
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        // Center action buttons: Copy + Save + Shelf
        let centerBtnH = scaled(30, minimum: 17)
        let centerGap = scaled(7, minimum: 4)
        ...
        let maxTitleW = max(
            ...
            (L("Shelf") as NSString).size(withAttributes: titleAttrs).width
        )
        ...
        let totalH = centerBtnH * 3 + centerGap * 2
        let btnsY = r.midY - totalH/2

        // Stacked top→bottom: Copy, Save, Shelf
        let copyRect  = NSRect(x: r.midX - centerBtnW/2, y: btnsY + (centerBtnH + centerGap) * 2, width: centerBtnW, height: centerBtnH)
        let saveRect  = NSRect(x: r.midX - centerBtnW/2, y: btnsY + (centerBtnH + centerGap),     width: centerBtnW, height: centerBtnH)
        let shelfRect = NSRect(x: r.midX - centerBtnW/2, y: btnsY,                                width: centerBtnW, height: centerBtnH)
        copyBtnRect  = copyRect
        saveBtnRect  = saveRect
        shelfBtnRect = shelfRect

        for (rect, title) in [(copyRect, L("Copy")), (saveRect, L("Save")), (shelfRect, L("Shelf"))] {
    ...
    override func mouseUp(with event: NSEvent) {
        ...
        if shelfBtnRect.contains(p)  { onAddToShelf?(); return }

        // Click anywhere else on the thumbnail body — open the editor.
        // (The ✕ corner button handles plain dismissal.)
        onEdit?()
    }
